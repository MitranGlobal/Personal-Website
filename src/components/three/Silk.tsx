...
import { Color, type Mesh, type IUniform, type ShaderMaterial } from 'three';
...
const SilkPlane = forwardRef<Mesh, { uniforms: SilkUniforms }>(function SilkPlane({ uniforms }, ref) {
  ...
  const local = ref as React.MutableRefObject<Mesh | null>;

  useLayoutEffect(() => {
    if (local.current) {
      local.current.scale.set(viewport.width, viewport.height, 1);
    }
  }, [local, viewport]);
...
export default function Silk({
  speed = 5,
  scale = 1,
  // Antique gold — the brand accent, held here deliberately
  color = '#C9A227',
  noiseIntensity = 1.5,
  rotation = 0,
  lightMode = false,
